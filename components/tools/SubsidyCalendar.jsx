...
function DdayBadge({ endDate, small = false }) {
  ...
  if (d < 0)       { bg = '#f0f0f0'; color = '#aaa';    border = '#ddd';     text = '마감'; }
  else if (d === 0){ bg = '#fff0f0'; color = '#e74c3c'; border = '#e74c3c';  text = 'D-Day'; }
  else if (d <= 7) { bg = '#fff0f0'; color = '#e74c3c'; border = '#e74c3c';  text = `D-${d}`; }
  else if (d <= 30){ bg = '#fff8e1'; color = '#f39c12'; border = '#f39c12';  text = `D-${d}`; }
  else             { bg = '#eef2fb'; color = '#1b4797'; border = '#b8ccf0';  text = `D-${d}`; }
  return (
    <span style={{
      padding: small ? '2px 7px' : '4px 10px',
      borderRadius: '20px', fontSize: small ? '10px' : '12px', fontWeight: 700,
...
function BottomSheet({ open, onClose, onSubmit }) {
  const EMPTY = { title: '', category: '자금지원', start_date: '', end_date: '', target: '', description: '', url: '', amount: '' };
  const [form, setForm] = useState(EMPTY);
  const [saving, setSaving] = useState(false);
  const [err, setErr] = useState('');

  useEffect(() => { if (!open) { setForm(EMPTY); setErr(''); } }, [open]);

  async function handleSubmit() {
    if (!form.title || !form.end_date) { setErr('제목과 마감일은 필수입니다'); return; }
    setSaving(true);
  ...
  return (
    <>
      ...
      <div style={{
        ...
        zIndex: 400, animation: 'slideUp 0.25s ease',
        maxHeight: '90dvh', overflowY: 'auto',
      }}>
        {/* 핸들 */}
        <div style={{ width: '40px', height: '4px', borderRadius: '2px', background: '#ddd', margin: '0 auto 18px' }} />
        <div style={{ fontSize: '17px', fontWeight: 700, marginBottom: '16px', color: 'var(--color-gray-900)' }}>
          지원금 등록
        </div>
        {err && <div style={{ padding: '10px 12px', background: '#fff0f0', color: 'var(--color-danger)', borderRadius: '8px', fontSize: '13px', marginBottom: '12px' }}>{err}</div>}

        <div style={{ fontSize: '12px', fontWeight: 600, color: 'var(--color-gray-700)', marginBottom: '5px' }}>제목 *</div>
        {inp('title', { placeholder: '예) 소상공인 경영안정자금 신청' })}

        <div style={{ fontSize: '12px', fontWeight: 600, color: 'var(--color-gray-700)', marginBottom: '5px' }}>카테고리 *</div>
        <select
          value={form.category}
          onChange={e => setForm(f => ({ ...f, category: e.target.value }))}
          style={{ width: '100%', padding: '10px 12px', fontSize: '14px', border: '1.5px solid var(--color-gray-300)', borderRadius: '8px', outline: 'none', fontFamily: 'inherit', marginBottom: '10px', background: '#fff' }}
        >
        ...
        <div style={{ display: 'flex', gap: '10px', marginBottom: '0' }}>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: '12px', fontWeight: 600, color: 'var(--color-gray-700)', marginBottom: '5px' }}>신청 시작일</div>
            {inp('start_date', { type: 'date' })}
          </div>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: '12px', fontWeight: 600, color: 'var(--color-gray-700)', marginBottom: '5px' }}>마감일 *</div>
            {inp('end_date', { type: 'date' })}
          </div>
        </div>

        <div style={{ fontSize: '12px', fontWeight: 600, color: 'var(--color-gray-700)', marginBottom: '5px' }}>신청 대상</div>
        {inp('target', { placeholder: '예) 소상공인 전체' })}

        <div style={{ fontSize: '12px', fontWeight: 600, color: 'var(--color-gray-700)', marginBottom: '5px' }}>지원금액</div>
        {inp('amount', { placeholder: '예) 최대 400만원' })}

        <div style={{ fontSize: '12px', fontWeight: 600, color: 'var(--color-gray-700)', marginBottom: '5px' }}>설명</div>
        <textarea
          value={form.description}
          onChange={e => setForm(f => ({ ...f, description: e.target.value }))}
          placeholder="지원사업 상세 설명"
          rows={3}
          style={{ width: '100%', padding: '10px 12px', fontSize: '14px', border: '1.5px solid var(--color-gray-300)', borderRadius: '8px', outline: 'none', fontFamily: 'inherit', marginBottom: '10px', resize: 'vertical' }}
        />

        <div style={{ fontSize: '12px', fontWeight: 600, color: 'var(--color-gray-700)', marginBottom: '5px' }}>신청 링크 URL</div>
...
export default function SubsidyCalendar() {
  ...
  useEffect(() => { loadSubsidies(); }, [year, month, category]);
  ...
  const selectedSubsidies = useMemo(() => {
    ...
    return subsidies.filter(s => s.end_date.slice(8, 10) === String(selectedDate).padStart(2, '0'));
  }, [selectedDate, subsidies]);

  // 리스트 뷰: D-day 임박 순 정렬, 마감 항목은 뒤로
  const sortedList = useMemo(() => {
    return [...subsidies].sort((a, b) => {
      const da = getDday(a.end_date), db = getDday(b.end_date);
  ...
  return (
    <div style={{ maxWidth: '480px', margin: '0 auto', padding: '20px 16px 80px', background: 'var(--color-bg)' }}>

      {/* ── 헤더 ── */}
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '20px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          <svg viewBox="0 0 48 48" width="44" height="44" fill="none">
            <rect x="4" y="8" width="32" height="30" rx="4" stroke="#1b4797" strokeWidth="2.4" fill="#eef2fb"/>
            <path d="M4 17h32" stroke="#1b4797" strokeWidth="2.2"/>
            <path d="M14 8V5M26 8V5" stroke="#1b4797" strokeWidth="2.4" strokeLinecap="round"/>
            <path d="M10 24h4M10 30h4M18 24h4" stroke="#4f80e1" strokeWidth="2" strokeLinecap="round"/>
            <circle cx="36" cy="36" r="10" fill="#eef2fb" stroke="#1b4797" strokeWidth="2.2"/>
            <path d="M36 30v6l3 3" stroke="#4f80e1" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
          <div>
            <div style={{ fontSize: '20px', fontWeight: 700, color: 'var(--color-gray-900)', lineHeight: 1.2 }}>지원금 캘린더</div>
            <div style={{ fontSize: '12px', color: 'var(--color-gray-500)', marginTop: '3px' }}>놓치면 아까운 소상공인 지원사업</div>
          </div>
        </div>
        <div style={{ fontSize: '13px', fontWeight: 700, color: 'var(--color-primary)', background: 'var(--color-primary-bg)', borderRadius: '8px', padding: '6px 12px', flexShrink: 0 }}>
          {year}년 {month}월
        </div>
      </div>
        ...
        <div>
          ...
          <div style={{ display: loading ? 'none' : 'block', background: '#fff', borderRadius: '20px', padding: '18px', boxShadow: 'var(--shadow-sm)', border: '1px solid var(--color-gray-200)', marginBottom: '12px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '14px' }}>
              ...
              <span style={{ fontSize: '16px', fontWeight: 700, color: 'var(--color-gray-900)' }}>{year}년 {month}월</span>
              <button onClick={nextMonth} style={{ width: '32px', height: '32px', border: 'none', background: 'var(--color-gray-100)', borderRadius: '8px', cursor: 'pointer', fontSize: '16px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>›</button>
            </div>

            {/* 요일 헤더 */}
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', marginBottom: '6px' }}>
              {WEEK_DAYS.map((d, i) => (
                <div key={d} style={{ textAlign: 'center', fontSize: '12px', fontWeight: 700, padding: '4px 0',
                  color: i === 0 ? 'var(--color-danger)' : i === 6 ? 'var(--color-accent)' : 'var(--color-gray-500)' }}>
                  {d}
                </div>
              ))}
            </div>
              ...
              {grid.map((day, idx) => {
                ...
                return (
                  <div key={idx} onClick={() => handleDayClick(day)} style={{
                    ...
                    borderRadius: '10px', cursor: hasSub ? 'pointer' : 'default',
                    background: isSelected ? 'var(--color-primary)' : isTod ? 'var(--color-primary-bg)' : 'transparent',
          ...
          {selectedDate && selectedSubsidies.length > 0 && (
            <div style={{ animation: 'slideUp 0.2s ease' }}>
              <div style={{ fontSize: '14px', fontWeight: 700, color: 'var(--color-gray-700)', marginBottom: '8px', paddingLeft: '4px' }}>
                ...
                return (
                  <div key={s.id} style={{
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
                      <span style={{ fontSize: '12px', fontWeight: 700, color: cat.color, background: cat.bg, padding: '2px 8px', borderRadius: '10px' }}>{s.category}</span>
                      <DdayBadge endDate={s.end_date} small />
                    </div>
                    <div style={{ fontSize: '15px', fontWeight: 700, color: 'var(--color-gray-900)', marginBottom: '4px' }}>{s.title}</div>
                    {s.amount && <div style={{ fontSize: '13px', color: 'var(--color-primary)', fontWeight: 600 }}>💰 {s.amount}</div>}
                  </div>
                );
              })}
            </div>
          ...
          {sortedList.filter(s => !showBookmarkOnly || bookmarks.includes(s.id)).map(s => {
            ...
            return (
              <div key={s.id} style={{
                ...
                {/* 제목 */}
                <div style={{ fontSize: '16px', fontWeight: 700, color: 'var(--color-gray-900)', marginBottom: '6px', lineHeight: 1.4 }}>
                  {s.title}
                </div>

                {/* 신청기간 + 대상 */}
                {(s.start_date || s.end_date) && (
                  <div style={{ fontSize: '13px', color: 'var(--color-gray-500)', marginBottom: '4px' }}>
                    📅 신청기간: {s.start_date ? `${s.start_date.slice(5).replace('-', '.')} ~ ` : ''}{s.end_date.slice(5).replace('-', '.')}
                  </div>
                )}
                {s.target && <div style={{ fontSize: '13px', color: 'var(--color-gray-500)', marginBottom: '4px' }}>👥 대상: {s.target}</div>}
                {s.amount && <div style={{ fontSize: '13px', color: 'var(--color-primary)', fontWeight: 600, marginBottom: '8px' }}>💰 {s.amount}</div>}

                {/* 설명 */}
                {s.description && (
                  <div style={{ fontSize: '13px', color: 'var(--color-gray-700)', lineHeight: 1.6, marginBottom: '12px', paddingTop: '10px', borderTop: '1px solid var(--color-gray-200)' }}>
                    {s.description}
                  </div>
                )}
      ...
      {/* ── 바텀 시트 등록 모달 ── */}
      <BottomSheet open={showAdd} onClose={() => setShowAdd(false)} onSubmit={loadSubsidies} />
