...
export async function GET(request, { params }) {
  try {
    const [rows] = await pool.query('SELECT * FROM subsidies WHERE id = ? AND is_active = 1', [params.id]);
...
export async function PUT(request, { params }) {
  try {
    ...
    await pool.query(
      `UPDATE subsidies SET title=?, category=?, start_date=?, end_date=?, target=?, description=?, url=?, amount=? WHERE id=?`,
...
export async function DELETE(request, { params }) {
  try {
    ...
    await pool.query('UPDATE subsidies SET is_active = 0 WHERE id = ?', [params.id]);
    return NextResponse.json({ success: true });
  } catch (error) {
    return NextResponse.json({ success: false, error: error.message }, { status: 500 });
  }
