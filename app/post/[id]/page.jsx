...
export default function PostDetail() {
  ...
  useEffect(() => { fetchPost(); fetchComments(); checkBookmark(); }, [id]);

  async function handleLikePost() { const r = await fetch('/api/posts/' + id + '/like', { method: 'POST' }); if (r.ok) { const d = await r.json(); setPost(p => p ? { ...p, alreadyLiked: d.liked, likeCount: d.likeCount } : p); } }
  async function handleDeletePost() { if (!confirm('이 글을 삭제하시겠습니까?')) return; const r = await fetch('/api/posts/' + id, { method: 'DELETE' }); if (r.ok) router.push('/'); }
  async function handleReport() { const r = await fetch('/api/posts/' + id + '/report', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ reason: '부적절한 내용' }) }); if (r.ok) alert('신고 완료'); setShowMenu(false); }
  async function handleBookmark() { const r = await fetch('/api/bookmarks', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ postId: parseInt(id) }) }); if (r.ok) { const d = await r.json(); setBookmarked(d.bookmarked); } }
  async function handleBlockUser() { if (!post) return; if (!confirm('이 사용자의 글을 앞으로 숨기시겠습니까?')) return; const r = await fetch('/api/blocks', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId: post.userId }) }); if (r.ok) { alert('차단 완료'); router.push('/'); } setShowMenu(false); }
  function handleShare() { const u = window.location.href; if (navigator.share) navigator.share({ title: post.title, url: u }); else navigator.clipboard.writeText(u).then(() => alert('링크가 복사되었습니다')); setShowMenu(false); }

  async function handleCommentSubmit() { if (!commentText.trim()) return; const r = await fetch('/api/comments', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ postId: parseInt(id), parentId: replyTo, content: commentText }) }); if (r.ok) { setCommentText(''); setReplyTo(null); fetchComments(); setPost(p => p ? { ...p, commentCount: p.commentCount + 1 } : p); } }
  async function handleCommentLike(cid) { const r = await fetch('/api/comments/' + cid + '/like', { method: 'POST' }); if (r.ok) fetchComments(); }
    ...
    <div style={{
      ...
      <div className='top-bar' style={{ position: 'relative', flexShrink: 0 }}>
        ...
        <div className='top-bar-right' style={{ position: 'relative' }}>
          <button className='top-bar-btn' onClick={handleBookmark}>{bookmarked ? '★' : '☆'}</button>
          <button className='top-bar-btn' onClick={handleShare}>
            <svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="#555" strokeWidth="2" strokeLinecap="round"><path d="M4 12v8a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-8"/><polyline points="16 6 12 2 8 6"/><line x1="12" y1="2" x2="12" y2="15"/></svg>
          </button>
          <button className='top-bar-btn' onClick={() => setShowMenu(!showMenu)}>
            <svg viewBox="0 0 24 24" width="18" height="18" fill="#555"><circle cx="12" cy="5" r="1.5"/><circle cx="12" cy="12" r="1.5"/><circle cx="12" cy="19" r="1.5"/></svg>
          </button>
          {showMenu && (
            <div className='dropdown-menu'>
              {(post.isAuthor || post.isAdmin) && <div className='dropdown-item danger' onClick={handleDeletePost}>삭제</div>}
              <div className='dropdown-item' onClick={handleReport}>신고</div>
              {!post.isAuthor && <div className='dropdown-item' onClick={handleBlockUser}>이 사용자 차단</div>}
            </div>
          )}
        </div>
      </div>
      ...
      <div style={{
        ...
        <div className='comment-input-inner'>
          <input className='comment-input' type='text' placeholder={replyTo ? '답글 작성...' : '댓글 작성...'} value={commentText} onChange={(e) => setCommentText(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') handleCommentSubmit(); }} />
