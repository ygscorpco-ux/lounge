...
export default function WritePage() {
  ...
  useEffect(() => {
    ...
    const onTouchMove = (e) => {
      if (el.scrollTop === 0 && e.touches[0].clientY > startY) e.preventDefault();
    };
  ...
  async function handleSubmit() {
    ...
    // 투표가 있으면 유효성 검사
    if (poll) {
      if (!poll.question.trim()) { setError("투표 질문을 입력해주세요"); return; }
      const validOptions = poll.options.filter((o) => o.trim());
      if (validOptions.length < 2) { setError("투표 항목을 2개 이상 입력해주세요"); return; }
    }
    ...
    const res = await fetch("/api/posts", {
      ...
      body: JSON.stringify({
        ...
        poll: poll ? { question: poll.question, options: poll.options.filter((o) => o.trim()) } : null,
      }),
    });
    ...
    <div style={{
      ...
      <div style={{
        ...
        <button onClick={handleSubmit} disabled={!canSubmit} style={{
          ...
          color: canSubmit ? "#1b4797" : "#ccc", fontWeight: 600, cursor: canSubmit ? "pointer" : "default",
        }}>
          완료
        </button>
      </div>
      ...
      {error && (
        <div style={{ padding: "10px 16px", background: "#fff5f5", color: "#e53e3e", fontSize: "13px", flexShrink: 0 }}>
          {error}
        </div>
      )}
      ...
      <div ref={scrollRef} style={{
        ...
        <input type="text" placeholder="제목을 입력해주세요." value={title}
          ...
          style={{
            display: "block", width: "100%", padding: "18px 16px 14px",
        ...
        <textarea ref={textareaRef}
          ...
          style={{
            ...
            border: "none", fontSize: "15px", lineHeight: 1.7, color: "#333",
            outline: "none", resize: "none", overflow: "hidden",
            background: "transparent", minHeight: "200px", boxSizing: "border-box",
          }}
        />
            ...
            {images.map((url, idx) => (
              <div key={idx} style={{ position: "relative", width: "100px", height: "100px" }}>
                ...
                <button onClick={() => removeImage(idx)} style={{
                  ...
                  width: "22px", height: "22px", color: "#fff", fontSize: "13px",
        ...
        {poll && (
          <div style={{ margin: "0 16px 16px", borderRadius: "12px", border: "1px solid #e8edf5", overflow: "hidden" }}>
            <div style={{
              ...
              <span style={{ fontSize: "14px", fontWeight: 700, color: "#1b4797" }}>📊 투표</span>
              <button onClick={() => setPoll(null)} style={{
                background: "none", border: "none", color: "#999", fontSize: "13px", cursor: "pointer",
              }}>삭제</button>
            </div>
            <div style={{ padding: "12px 16px", display: "flex", flexDirection: "column", gap: "10px" }}>
              {/* 투표 질문 */}
              <input type="text" placeholder="투표 질문을 입력하세요" value={poll.question}
              ...
              {poll.options.map((opt, idx) => (
                <div key={idx} style={{ display: "flex", gap: "8px", alignItems: "center" }}>
                  <span style={{ fontSize: "13px", color: "#999", width: "20px", flexShrink: 0 }}>{idx + 1}</span>
                  <input type="text" placeholder={`항목 ${idx + 1}`} value={opt}
                  ...
                  {poll.options.length > 2 && (
                    <button onClick={() => removePollOption(idx)} style={{
              ...
              {poll.options.length < 4 && (
                <button onClick={addPollOption} style={{
                  padding: "9px", border: "1px dashed #c0cde0", borderRadius: "8px",
                  background: "none", fontSize: "13px", color: "#1b4797", cursor: "pointer",
                }}>
                  + 항목 추가
                </button>
              )}
      ...
      <div style={{
        flexShrink: 0, borderTop: "1px solid #f0f0f0",
        ...
        {/* 이미지 첨부 버튼 */}
        <button onClick={() => !uploading && images.length < 4 && fileInputRef.current.click()}
          style={{
        ...
        >
          ...
          {images.length > 0 && (
            <span style={{ fontSize: "12px", color: "#1b4797", fontWeight: 600 }}>{images.length}/4</span>
          )}
        </button>
        ...
        {uploading && (
          <span style={{ fontSize: "12px", color: "#999", marginLeft: "4px" }}>업로드 중...</span>
        )}
      </div>
    </div>
  );
